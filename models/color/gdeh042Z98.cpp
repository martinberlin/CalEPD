...
// Constructor
Gdeh042Z98::Gdeh042Z98(EpdSpi &dio) :
        Adafruit_GFX(GDEH042Z98_WIDTH, GDEH042Z98_HEIGHT),
        Epd(GDEH042Z98_WIDTH, GDEH042Z98_HEIGHT), IO(dio) {
    printf("Gdeh042Z98() constructor injects IO and extends Adafruit_GFX(%d,%d)\n",
           GDEH042Z98_WIDTH, GDEH042Z98_HEIGHT);
}

//Initialize the display
void Gdeh042Z98::init(bool debug) {
    debug_enabled = debug;
    if (debug_enabled) printf("Gdeh042Z98::init(%d) and reset EPD\n", debug);
...
void Gdeh042Z98::fillScreen(uint16_t color) {
    uint8_t black = GDEH042Z98_8PIX_WHITE;
    uint8_t red = GDEH042Z98_8PIX_RED_WHITE;
    if (color == EPD_WHITE) {

    } else if (color == EPD_BLACK) {
        black = GDEH042Z98_8PIX_BLACK;
        printf("fillScreen BLACK SELECTED\n");
    } else if (color == EPD_RED) {
        red = GDEH042Z98_8PIX_RED;
...
void Gdeh042Z98::update() {
    ...
    uint16_t i = 0;
    uint8_t xLineBytes = GDEH042Z98_WIDTH / 8;
    uint8_t x1buf[xLineBytes];
    // Curiosity doing it x++ is mirrored

    for (uint16_t y = 1; y <= GDEH042Z98_HEIGHT; y++) {
        for (uint16_t x = 1; x <= xLineBytes; x++) {
            uint8_t data = i < sizeof(_black_buffer) ? _black_buffer[i] : GDEH042Z98_8PIX_WHITE;
            x1buf[x - 1] = data;
    ...
    for (uint16_t y = 1; y <= GDEH042Z98_HEIGHT; y++) {
        for (uint16_t x = 1; x <= xLineBytes; x++) {
            uint8_t data = i < sizeof(_red_buffer) ? _red_buffer[i] : GDEH042Z98_8PIX_RED_WHITE;
            x1buf[x - 1] = data;
            if (x == xLineBytes) {
                IO.data(x1buf, sizeof(x1buf));
            }
...
void Gdeh042Z98::_waitBusy(const char *message) {
    if (debug_enabled) {
...
void Gdeh042Z98::_sleep() {

    IO.cmd(0x10);
    IO.data(0x01);// power off
}

void Gdeh042Z98::_rotate(uint16_t &x, uint16_t &y, uint16_t &w, uint16_t &h) {
    switch (getRotation()) {
        case 1:
            swap(x, y);
            swap(w, h);
            x = GDEH042Z98_WIDTH - x - w - 1;
            break;
        case 2:
            x = GDEH042Z98_WIDTH - x - w - 1;
            y = GDEH042Z98_HEIGHT - y - h - 1;
            break;
        case 3:
            swap(x, y);
            swap(w, h);
...
void Gdeh042Z98::drawPixel(int16_t x, int16_t y, uint16_t color) {
    ...
    switch (getRotation()) {
        case 1:
            swap(x, y);
            x = GDEH042Z98_WIDTH - x - 1;
            break;
        case 2:
            x = GDEH042Z98_WIDTH - x - 1;
            y = GDEH042Z98_HEIGHT - y - 1;
            break;
        case 3:
            swap(x, y);
            y = GDEH042Z98_HEIGHT - y - 1;
    ...
    uint16_t i = x / 8 + y * GDEH042Z98_WIDTH / 8;
    ...
    _black_buffer[i] = (_black_buffer[i] & (GDEH042Z98_8PIX_WHITE ^ (1 << (7 - x % 8)))); // white
    _red_buffer[i] = (_red_buffer[i] & (GDEH042Z98_8PIX_RED ^ (1 << (7 - x % 8)))); // white
